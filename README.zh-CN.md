...
<p align="center">
  <img alt="Version" src="https://img.shields.io/badge/version-0.1.3-00B0FF?style=flat-square">
...
<p align="center">
  <a href="https://github.com/yanjingtui/Sift/releases/latest">
    <img src="https://img.shields.io/badge/下载-Sift%20v0.1.3-00B0FF?style=for-the-badge" alt="下载 Sift">
  </a>
</p>
...
| 格式 | 浏览 | 评级存储 |
|:----:|:----:|---------|
| **JPEG**（`jpg` / `jpeg`） | ✅ | XMP 内嵌文件（APP1 段） |
| **PNG** | ✅ | XMP 内嵌文件（iTXt chunk） |
| **HEIC** | ✅ | `.xmp` 旁车文件 —— 原图不动 |
| **WebP** | ✅ | `.xmp` 旁车文件 —— 原图不动 |
| RAW（CR3、NEF、ARW、RAF）、TIFF、GIF、BMP 等 | ❌ | 浏览器中不显示 |

- **JPEG 和 PNG** 评级直接写入文件内部 —— 拷贝、上传、重命名都跟着走。
- **HEIC 和 WebP** 评级写入同目录的 `.xmp` 旁车文件。Sift 拷贝/删除照片时会自动带上 sidecar；但如果你在 Sift 外移动/重命名图片，请把 `.xmp` 一起带走。
- **其他格式在写入前就被过滤掉**，绝不会触碰不支持的文件。最后一道保险：写入时会校验文件头魔数（不只看扩展名），任何无法识别的文件都退回 sidecar 模式 —— 改名的文件也不会损坏。
